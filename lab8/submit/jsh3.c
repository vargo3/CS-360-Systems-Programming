...
int main(int argc, char **argv){
	...
	int dummy, com_len, i, child_pid, pipe_child_pid, fd[2];
		...
		if (child_pid == 0){
			...
			while (i < STR_SIZE){
				...
				if (com_v[i] == NULL) //end of argument list
					...
					close(fd[0]); //release file descriptor
				}
				else if (strcmp(com_v[i], ">") == 0){ //Redirect stdout to file given as next argument
					temp = strtok(NULL, " "); //get file name
					fd[0] = open(temp, O_WRONLY | O_TRUNC | O_CREAT, 0644);
					if (fd[0] < 0){
						perror(temp);
						return 0;
					} //file open
					...
					temp = strtok(NULL, " "); //get file name
					fd[0] = open(temp, O_WRONLY | O_APPEND | O_CREAT, 0644);
					if (fd[0] < 0){
						perror(temp);
						return 0;
					} //file open
				...
				else if (strcmp(com_v[i], "|") == 0){ //Pipe output of the previous command to the input of the next command.
					if (pipe(fd) != 0){
						perror("Pipe");
						return 0;
					}
					...
					if (pipe_child_pid == 0){
						//Child process
						if (dup2(fd[1], 1) != 1){ //writes to pipe
							perror(temp);
							return 0;
						}
						close(fd[0]); //release pipe file descriptors
						close(fd[1]);

						com_v[i] = NULL;
						break; //all arguments read for this process. do exec
					}
					else{
						//Parent process
						if (user_command[com_len-1] != '&'){
							while (wait(&dummy) != pipe_child_pid); //wait until its child returns. Each child process will wait on its own children if needed
						}
						if (dup2(fd[0], 0) != 0){ //reads from pipe
							perror(temp);
							return 0;
						}
						close(fd[0]); //release pipe file descriptors
						...
						i = 0; //this will start com_v back at 0 and write over the previous command arguments for the new process to start
						continue;
					}
				}
				else{
